...
class MyInterface:
    ...
    def setup_ui(self):
        ...
        for i in range(3):
            ...
            wind_speed_label.grid(row=i, column=2, padx=5, pady=5)
        ...
        # Version label
        version_label = ctk.CTkLabel(footer_frame, text="Version 1.0.0", anchor="e")
    ...
    def update_wind_speed_live(self, ser, index):
        while self.live_update_flag:
            try:
                ...
                wind_speed = float(line.split(",")[2])
                self.update_wind_speed_display(index, wind_speed)
    ...
    def on_close(self):
        # Close serial connections before closing the application
        for ser in self.anemometers:
            ser.close()
        self.master.destroy()
...
def create_about_dialog(root):
    ...
    version_label = ctk.CTkLabel(master=content_frame,
                                text="Version: 1.0.0",  # Update version number
                                font=("Arial", 12, "bold"))
    version_label.pack()
